...
- Stable usage instructions belong in `PhenotypeUsageGuide`, which is bound to a `Phenotype` and referenced by compile artifacts, generation jobs, versions, and output references by guide id/revision.
...
@dna/core domain model + @dna/storage repository ports
        ...
        +--> Results: Phenotype, PhenotypeUsageGuide, PhenotypeVersion, AssetIndex, OutputReference
...
`PhenotypeUsageGuide` is the stable usage guide for a `Phenotype`, not version feedback, review metadata, or copied output-reference data. `dna phenotype guide prepare|create|update|show|render|list|archive` provides the preview/apply write surface. Formal phenotype compile and generation include the active guide id, revision, summary, usage scenarios, must-preserve / must-avoid rules, variant plan, production hints, and review checklist in the `PhenotypeCompileArtifact` and `GenerationJob.inputSnapshot`. Missing guides produce a warning instead of blocking generation, and new generated versions do not rewrite the guide unless the user explicitly updates it.
...
Managed generation execution is recorded on `GenerationJob`; DNA does not create a separate run object. Jobs carry `executionMode`, `provenanceLevel`, bounded `requestEvidence`, bounded `outputEvidence`, and `verificationSummary`. External/manual asset links are at most `external-linked`; only managed runner output that is verified can become `runner-verified`. `dna generation-job show|run|verify` previews by default where it writes evidence, and `dna phenotype generate --task <taskId> --runner mock-runner --apply` is the MVP shortcut for producing a runner-recorded job. `PhenotypeVersion.status = accepted` is still a lifecycle decision; strict acceptance can require runner-verified evidence, while warn mode only reports the gap.

Generation planning is a production orchestration layer above manual generation. `PhenotypeGenerationPlan` covers graph, species-group, species-node, or phenotype scope and expands deterministically into `PhenotypeGenerationTask` records for planned phenotypes. Planned phenotypes can use `productionSliceRole` to distinguish several production slices for the same graph/node/type, while duplicate default slices are still rejected. Tasks may also be created without a plan and can carry structured, domain-neutral `productionIntent` synthesized from usage guides, planned phenotypes, species nodes, and explicit plan/task overrides. `versionBinding` defaults to `latest-at-execution`; pinned node versions or compile artifacts are allowed but stale artifacts require explicit historical replay. `dna generation-plan update` and `dna generation-task update` preview orchestration metadata changes by default and apply only with `--apply` or `--yes`; they may update non-sensitive planning metadata, tags, requirements, preferences, notes, production intent, and task blocking state, but not immutable target identity or execution trace links. `dna phenotype generate --task <taskId> --apply` writes the generated compile artifacts, `GenerationJob`, and `PhenotypeVersion` back to the task for reviewable traceability, including the production intent used in compile and job snapshots. Plans and tasks can store non-sensitive model/tool preference, `llmInstructions`, `operatorNotes`, metadata, and extensions, but never provider credentials or complete private links.

Reference generation is separate from phenotype generation. `dna reference-generation prepare|run-mock` creates graph- or species-group-scoped prompts and optional persisted `EntityCompileArtifact` plus reference `GenerationJob` records without creating synthetic `SpeciesNode`, `Phenotype`, or `PhenotypeVersion` objects. `dna reference-generation link-asset` registers a safe pointer in `AssetIndex` for a reference job; it infers storage type from safe URI schemes such as `eagle://`, `local://`, `nas://`, Git, URL, and object-storage URIs, does not copy binaries, and rejects private or credential-bearing asset URIs. External outputs can be marked complete with `dna reference-generation complete --job <id> --apply` after active linked asset evidence exists, or atomically during `link-asset --mark-generated --apply`; completion stores only safe notes/tool labels/metadata and preserves the original prompt, target, and input snapshots. If a temporary pointer later moves to Eagle or another catalog, `dna reference-generation replace-asset --job <id> --old-asset <id> --new-asset-id <id> --uri eagle://... --apply` creates a new active pointer, archives the old pointer as historical evidence, and updates current completion evidence without creating phenotype records. Later phenotype tasks should cite reference jobs/assets by ids in requirements or metadata instead of copying private URLs.

### Local HTTP API And Web Explorer

DNA includes a local HTTP API baseline for health checks, graph tree data, generation plan/task data, and read-only explorer snapshots. HTTP access to the DNA web page is disabled by default. When explicitly enabled with `dna serve --web` or `createDnaHttpHandler(store, { webEnabled: true })`, it serves the DNA Read-only Explorer from `/api/workbench/snapshot`. The default page is `Atlas Map`, with `Graph Explorer`, `Generation Board`, `Phenotype Library`, and a shared `Inspector`; additional read-only view endpoints expose `/api/workbench/graph-map`, `/api/workbench/graphs/:graphId`, `/api/workbench/generation`, and `/api/workbench/library`. The Explorer shows safe result/gallery previews, graph-level DesignRelationship routes, graph interiors, usage guide coverage, generation trace paths, and mobile-friendly read-only navigation. It does not accept, reject, archive, run tasks, apply sync, or otherwise durably mutate DNA records. The legacy `/api/workbench/phenotypes` endpoint remains available as a narrow generated-result snapshot for existing local integrations.
...
- `PhenotypeUsageGuide` - stable usage guide bound to a phenotype, with lightweight revision
...
The stable exchange contract includes change-sets, proposals, facets, contexts, templates, libraries, atlases, graph lineage, species groups, phenotype usage guides, compile artifacts, generation plans, generation tasks, generation jobs, output references, reviews, and impacts. The full directory map is maintained in [docs/design/system-architecture.md](docs/design/system-architecture.md).
...
The CLI includes `dna graph tree --id <graph_id>` for a readable species tree, `dna graph tree --include-groups` for reviewer-visible group overlays, and `dna graph tree --include-phenotypes` for planned phenotype coverage. It also includes `dna compile atlas|graph|group|species|phenotype` for layered compile previews/persisted artifacts, `dna phenotype guide prepare|create|update|show|render|list|archive` for stable usage guides, `dna generation-plan create/list/show/update/expand`, `dna generation-task create/list/show/update/run-mock/link-result`, `dna reference-generation prepare|run-mock|link-asset|complete|replace-asset`, `dna phenotype-version ...` for candidate/accepted/replaced/rolled-back lifecycle metadata, `dna facet definition/schema/assignment ...` for facet write paths, `dna modeling check` for reusable modeling-quality review, `dna changeset list/show/review/apply/discard` for preview review, `dna proposal import-batch --in <file>` for `dna.modeling-batch.v1` initial modeling drafts, `dna sync export/import` for explicit directory exchange, and `dna serve` for the local HTTP API. Root CLI version discovery uses `dna --cli-version` or `dna -V`, while subcommands may own their domain `--version` options. Web page access remains off unless `dna serve --web` is used; when enabled, it is a read-only local Explorer with Atlas Map, Graph Explorer, Generation Board, Phenotype Library, and Inspector surfaces.
`dna.modeling-batch.v1` can declare graphs, species groups, design relationships, facets, and `phenotypePlans`; a phenotype plan may include `productionSliceRole` when several planned outputs share the same graph/node/type but serve different production slices. Default import mode creates a proposal with preview change-sets and a compact review report. Explicit `--mode draft-write` writes local seed objects through the service boundary and reports that it skips proposal review.
For formal generation, `dna phenotype generate --graph <id> --node <id> --type <type> --name <name> --brief <brief>` compiles or reuses layered species and phenotype artifacts; add `--apply` to persist the generated artifacts, phenotype version, and generation job. If the phenotype has an active usage guide, generation records the guide id/revision summary in artifacts, jobs, and versions; if not, it records a warning and does not invent one. Use `dna phenotype generate --task <taskId> --apply` when the work should update a generation task with artifact/job/version links and structured production intent. Stale supplied artifacts require explicit historical replay. `dna output-ref add` previews by default and persists only with command-local `--apply` or global `--yes`. Graph/group reference prompts use `dna reference-generation prepare|run-mock|link-asset|complete|replace-asset`; they create scoped reference `GenerationJob` records and safe `AssetIndex` pointers without creating synthetic phenotype records.
...
- 稳定使用说明属于绑定到 `Phenotype` 的 `PhenotypeUsageGuide`；compile artifact、generation job、version 和 output reference 只引用 guide id/revision，不复制成新的说明真源。
...
`PhenotypeUsageGuide` 是稳定绑定到 `Phenotype` 的使用说明，不是 version feedback、review metadata 或 output reference metadata。`dna phenotype guide prepare|create|update|show|render|list|archive` 提供默认 preview、显式 apply 的写入面。正式 phenotype compile 和 generation preparation 会把 active guide 的 id、revision、summary、usage scenarios、must preserve / must avoid、variant plan、production hints 和 review checklist 摘要写入 `PhenotypeCompileArtifact` 与 `GenerationJob.inputSnapshot`。缺少 guide 只产生 warning，不阻断生成，也不会因为生成新版本自动改写 guide。
...
受管生成执行继续记录在 `GenerationJob` 上，不新增独立 run 对象。Job 包含 `executionMode`、`provenanceLevel`、有界 `requestEvidence`、有界 `outputEvidence` 和 `verificationSummary`。外部/手工链接资产最多是 `external-linked`；只有 managed runner 输出并完成验证后才能成为 `runner-verified`。`dna generation-job show|run|verify` 中涉及写 evidence 的命令默认 preview；`dna phenotype generate --task <taskId> --runner mock-runner --apply` 是 MVP 的 runner-recorded 快捷路径。`PhenotypeVersion.status = accepted` 仍是生命周期决策；strict accept 可要求 runner-verified evidence，warn 模式只报告缺口。

生成计划是手动生成之上的生产编排层。`PhenotypeGenerationPlan` 覆盖 graph、species-group、species-node 或 phenotype 粒度，并确定性展开为面向 planned phenotypes 的 `PhenotypeGenerationTask`；planned phenotype 可以用 `productionSliceRole` 区分同一 graph/node/type 下的多个生产切片，重复 default slice 仍会被拒绝。task 也可以不依赖 plan 独立创建，并可保存从 usage guide、planned phenotype、species node 和显式 plan/task override 合成出的领域中立 `productionIntent`。`versionBinding` 默认是 `latest-at-execution`；也可 pinned 到 NodeVersion 或 compile artifact，但 stale artifact 需要显式 historical replay。`dna generation-plan update` 和 `dna generation-task update` 默认 preview，只有 `--apply` 或 `--yes` 才写入；它们可修改非敏感编排 metadata、tags、requirements、preference、notes、production intent 和 task blocking 状态，但不得修改不可变目标身份或执行 trace links。`dna phenotype generate --task <taskId> --apply` 会把生成出的 compile artifacts、`GenerationJob` 和 `PhenotypeVersion` 回写到 task，并记录当次 production intent，便于审阅追踪。plan/task 可以保存非敏感 model/tool preference、`llmInstructions`、`operatorNotes`、metadata 和 extensions，但不得保存 provider credentials 或完整私密链接。

参考生成独立于表型生成。`dna reference-generation prepare|run-mock` 面向 graph 或 species-group 生成参考提示，并可显式持久化 `EntityCompileArtifact` 与 reference `GenerationJob`；它不会创建 synthetic `SpeciesNode`、`Phenotype` 或 `PhenotypeVersion`。`dna reference-generation link-asset` 只登记安全 `AssetIndex` 指针，会从 `eagle://`、`local://`、`nas://`、Git、URL、对象存储等安全 URI scheme 推断 storage type，不复制二进制，并拒绝私密或携带凭据的 asset URI。外部产出的参考图可在已有 active linked asset evidence 后通过 `dna reference-generation complete --job <id> --apply` 标记完成，也可用 `link-asset --mark-generated --apply` 在同一事务中创建 asset 指针并更新 job；completion 只保存安全 note/tool label/metadata，并保留原 prompt、target 和 input snapshots。如果临时 local pointer 后续迁入 Eagle 或其他目录，`dna reference-generation replace-asset --job <id> --old-asset <id> --new-asset-id <id> --uri eagle://... --apply` 会创建新的 active pointer、把旧 pointer 归档为历史证据，并更新 current completion evidence，不创建 phenotype records。后续 phenotype task 应通过 requirements 或 metadata 引用 reference job/asset ids，而不是复制私密 URL。

### 本地 HTTP API 与 Web Explorer

DNA 已包含本地 HTTP API 基线，用于 health check、图谱树数据、generation plan/task 数据和只读 Explorer 快照。通过 HTTP 访问 DNA 网页默认关闭；使用 `dna serve --web` 或 `createDnaHttpHandler(store, { webEnabled: true })` 显式开启后，提供从 `/api/workbench/snapshot` 读取数据的 DNA Read-only Explorer。默认首页是 `Atlas Map`，并提供 `Graph Explorer`、`Generation Board`、`Phenotype Library` 与共享 `Inspector`；额外只读视图接口包括 `/api/workbench/graph-map`、`/api/workbench/graphs/:graphId`、`/api/workbench/generation` 和 `/api/workbench/library`。Explorer 支持安全结果/图库预览、graph-level DesignRelationship 路线、图谱内部结构、usage guide 覆盖情况、生成追溯链和移动端只读导航。Web 不接受、拒绝、归档、运行 task、apply sync 或以其他方式持久修改 DNA 记录。旧 `/api/workbench/phenotypes` 仍作为窄范围生成结果 snapshot 供既有本地集成读取。
...
- `PhenotypeUsageGuide` - 绑定到 phenotype 的稳定使用说明，使用轻量 revision
...
当前稳定交换契约覆盖 change-set、proposals、facets、contexts、templates、libraries、atlases、图谱谱系、species groups、phenotype usage guides、compile artifacts、generation plans、generation tasks、generation jobs、output references、reviews 和 impacts。完整目录结构以 [docs/design/system-architecture.md](docs/design/system-architecture.md) 为准。
...
CLI 提供 `dna graph tree --id <graph_id>` 输出可读物种树，`dna graph tree --include-groups` 可显示 group overlay，`dna graph tree --include-phenotypes` 可显示 planned phenotype 覆盖；提供 `dna compile atlas|graph|group|species|phenotype` 做 layered compile preview/persisted artifacts；提供 `dna phenotype guide prepare|create|update|show|render|list|archive` 管理稳定使用说明；提供 `dna generation-plan create/list/show/update/expand`、`dna generation-task create/list/show/update/run-mock/link-result` 做生成计划和任务编排；提供 `dna reference-generation prepare|run-mock|link-asset|complete|replace-asset` 做 graph/group scoped reference generation；提供 `dna phenotype-version ...` 治理 candidate/accepted/replaced/rolled-back 生命周期 metadata；提供 `dna facet definition/schema/assignment ...` 做 facet 写入路径；提供 `dna modeling check` 做可复用建模质量检查；提供 `dna changeset list/show/review/apply/discard` 做 preview 审阅确认；提供 `dna proposal import-batch --in <file>` 导入 `dna.modeling-batch.v1` 初始建模草案；提供 `dna sync export/import` 做显式目录交换，也提供 `dna serve` 启动本地 HTTP API。根 CLI 版本使用 `dna --cli-version` 或 `dna -V` 查看，子命令可继续拥有自己的领域 `--version` 参数。网页访问默认关闭；使用 `dna serve --web` 开启后，它是包含 Atlas Map、Graph Explorer、Generation Board、Phenotype Library 和 Inspector 的只读本地 Explorer。
`dna.modeling-batch.v1` 可声明 graphs、species groups、DesignRelationship、facets 与 `phenotypePlans`；当同一 graph/node/type 需要多个计划产物切片时，phenotype plan 可包含 `productionSliceRole`。默认导入会创建 proposal + preview change-sets，并输出紧凑审阅报告；显式 `--mode draft-write` 通过 service boundary 写入本地 seed objects，并说明它跳过 proposal review。
正式生成使用 `dna phenotype generate --graph <id> --node <id> --type <type> --name <name> --brief <brief>`，该命令会编译或复用 layered species/phenotype artifacts；加 `--apply` 才会持久化生成 artifacts、phenotype version 和 generation job。如果 phenotype 有 active usage guide，生成会记录 guide id/revision 摘要；如果没有，只记录 warning，不编造 guide。需要更新生成任务时使用 `dna phenotype generate --task <taskId> --apply`，它会把 artifact/job/version links 回写到 task，并记录当次 production intent。传入 stale artifact 时需要显式 historical replay。`dna output-ref add` 默认只 preview，只有命令级 `--apply` 或全局 `--yes` 才持久化。graph/group 粒度参考提示使用 `dna reference-generation prepare|run-mock|link-asset|complete|replace-asset`，只写入 scoped reference `GenerationJob` 和安全 `AssetIndex` 指针，不创建伪造的 phenotype records。
