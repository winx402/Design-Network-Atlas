...
Design Readiness scores whether an atlas, graph, species group, species node, or phenotype has enough design language for reliable LLM generation. Readiness is embedded in compile frames/artifacts, not stored as a durable graph fact or cache table, so stale dependency vectors make the score stale too. `dna readiness assess --target-type <type> --target <id>` previews a new compile/readiness artifact, and `--apply` persists only that artifact. The policy is `off | warn | block`: off ignores readiness, warn is the default non-blocking reminder, and block stops formal phenotype generation when critical readiness is missing, stale, or blocked.

Self-Optimization detects reusable design-language maintenance candidates from feedback, review notes, compile feedback, and human discussion. It maps candidates to the DNA concept map, proposes a safe write location such as usage guide, context, facet, species constraint, relationship contract, or generation task metadata, and defaults to a proposal/change-set preview. It never silently writes graph/context/facet/relationship facts, and low-confidence feedback remains feedback or an open question.
...
Design Readiness 用来判断 atlas、graph、species group、species node 或 phenotype 是否具备足够设计语言支撑 LLM 生成。readiness 嵌入 compile frame / artifact，不是图谱事实，也不是独立缓存表；dependency vector stale 时分数也视为 stale。`dna readiness assess --target-type <type> --target <id>` 默认预览新的 compile/readiness artifact，`--apply` 只持久化 artifact。策略为 `off | warn | block`：off 忽略 readiness，warn 是默认非阻断提醒，block 会在关键 readiness 缺失、stale 或 blocked 时阻断正式 phenotype generation。

Self-Optimization 从反馈、审阅笔记、compile feedback 和人工讨论中识别可复用的设计语言维护候选。它基于 DNA concept map 建议安全写入位置，例如 usage guide、context、facet、species constraint、relationship contract 或 generation task metadata，并默认输出 proposal/change-set preview。它不会静默写入 graph/context/facet/relationship facts；低置信度反馈继续留在 feedback 或 open question。
