...
| `codex-skills/dna` | Agent skill router for DNA scenario work | Routes to workflow skills and keeps persistence in CLI/service layers |
...
| `codex-skills/dna` | DNA 场景工作的 skill 路由入口 | 分流到具体 workflow skill，持久化仍由 CLI/service 完成 |
...
DNA 当前是本地优先的 v0.6.0 项目，适合试点使用、本地设计图谱治理、图谱建模/编辑引导 skill、图谱树检查、preview change-set 审阅确认、prompt / brief 生成、结果库路由、输出引用管理、审查记录、影响分析、Git 友好交换、本地 HTTP API 集成、工作台数据快照，以及历史结果库图谱绑定的升级修复。
