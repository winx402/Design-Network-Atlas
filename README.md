...
DNA is a local-first, open-source system for design genome graphs. It helps teams model reusable design dimensions, stable design species, evolution relationships, generated phenotypes, output references, optional phenotype libraries, reviews, and downstream impact.
...
- **Phenotype layer**: concrete generated or curated outputs for a specific task, with versions, reviews, output references, and optional library bindings.

DNA does not store binary assets by default. It stores output references and asset indexes such as local paths, Git paths, database record pointers, Eagle links, Figma links, object-storage URLs, or generated-output references. A `PhenotypeLibrary` is optional: teams can use DNA as a unified catalog, or simply point generated results to an existing repository, DAM, NAS, Eagle library, database, or engine export.
...
- `dna` CLI for graph, template, node, edge, phenotype, asset, output-ref, library, review, impact, import, export, and version-history workflows.
...
- Use a phenotype library as a human-searchable catalog, while keeping graph IDs and library IDs many-to-many and decoupled.
- Record generated outputs without adopting DNA's library layer by pointing directly to Git, URL, database, object storage, or another external system.
...
Create an optional phenotype library and bind it to a graph:

```bash
pnpm dna --db .dna/dna.sqlite library create \
  --id lib-shared-ui \
  --name "Shared UI Library" \
  --purpose "shared phenotype catalog" \
  --profile media-asset \
  --accepted-reference eagle \
  --yes

pnpm dna --db .dna/dna.sqlite library bind-graph \
  --id bind-ui \
  --library lib-shared-ui \
  --graph graph-demo \
  --role primary-library \
  --yes

pnpm dna --db .dna/dna.sqlite library mount add \
  --id mount-eagle \
  --library lib-shared-ui \
  --storage-type eagle \
  --adapter-kind managed-library \
  --name "Eagle Main" \
...
Record output references with or without a phenotype library:

```bash
pnpm dna --db .dna/dna.sqlite output-ref add \
  --id out-git-warning \
  --graph graph-demo \
  --phenotype-version pv-example \
...
pnpm dna --db .dna/dna.sqlite output-ref add \
  --id out-eagle-warning \
  --graph graph-demo \
  --phenotype-version pv-example \
  --uri "eagle://item/warning" \
  --type eagle \
  --role preview \
  --library lib-shared-ui \
  --storage-mount mount-eagle \
  --tag "UI/Icon" \
  --normalized-tag ui-icon \
  --yes
```
...
DNA 是一个本地优先的开源“设计基因图谱”系统，用来管理设计维度、稳定设计对象、演化关系、生成结果、输出引用、可选表型库、审查记录和下游影响。
...
- **表型层**：定义某次具体任务中生成或整理出的结果实例，包含版本、审查、输出引用和可选库绑定。

DNA 默认不保存二进制素材，只保存输出引用和素材索引，例如本地路径、Git 路径、数据库记录指针、Eagle 链接、Figma 链接、对象存储 URL 或生成结果引用。`PhenotypeLibrary` 是可选能力：团队可以把 DNA 当成统一目录，也可以只把生成结果指向现有 Git 仓库、DAM、NAS、Eagle 库、数据库或引擎导出目录。

## 当前能力

- TypeScript monorepo，包含 core、storage、SQLite、CLI、template-pack、server adapter 和 web workbench。
- SQLite 本地存储，支持导入导出。
- `dna` CLI 支持 graph、template、node、edge、phenotype、asset、output-ref、library、review、impact、import、export、version-history。
...
- 把表型库作为人和 Agent 都能搜索的统一目录，同时保持知识图谱 ID 和表型库 ID 多对多解耦。
- 不使用 DNA 表型库时，也可以直接把生成结果指向 Git、URL、数据库、对象存储或其他外部系统。
