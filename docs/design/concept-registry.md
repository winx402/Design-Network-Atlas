...
| `AssetIndex` | external-pointer model | Indexes a physical or external asset pointer, such as file, library item, preview, source, runtime export, variant, or scoped reference asset. | External pointer; no binary ownership. | Direct audit/admin write through CLI/application service. | `graphs/<graph_id>/assets/` | `OutputReference`, `PhenotypeVersion`, `GenerationJob` |
