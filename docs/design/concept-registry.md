...
- Output/Provenance Relationship: `Phenotype`, `PhenotypeUsageGuide`, `PhenotypeVersion`, `GenerationJob`, `OutputReference`, and `AssetIndex` record generation lineage, stable usage guidance, outputs, and external pointers.
...
| `PhenotypeCompileArtifact` | compiled view model | Compiles inherited species frames plus a phenotype frame into prompt, negative prompt, brief, generation constraints, review checklist, dependency vector, decisions, feedback, and optional usage guide snapshot. | Compiled view; preview for standalone compile, direct audit write during formal generation apply. | Application service or CLI compile/generate flows. | `graphs/<graph_id>/compile/phenotypes/` | `SpeciesCompileArtifact`, `PhenotypeUsageGuide`, `PhenotypeVersion` |
| `Phenotype` | output provenance model | Stable generated or curated result object for a species and phenotype type. | Generated output container; archived by status. | Application service or CLI generated-output flows. | `graphs/<graph_id>/phenotypes/` | `PhenotypeUsageGuide`, `PhenotypeVersion`, `OutputReference` |
| `PhenotypeUsageGuide` | output provenance model | Stable usage contract for a Phenotype, including usage scenarios, how to use it, design-language sources, must-preserve/must-avoid rules, variant plan, production hints, and review checklist. | One active guide per phenotype. Updates increment lightweight `revision`; no separate guide version table. Missing guide is a generation warning, not a blocker. | Application service or CLI phenotype guide flows. | `graphs/<graph_id>/phenotypes/<phenotype_id>/usage-guide.json` and `.md` | `Phenotype`, `PhenotypeCompileArtifact`, `GenerationJob.inputSnapshot`, `PhenotypeVersion`, `OutputReference` |
| `PhenotypeVersion` | output provenance model | Immutable generated output snapshot. Formal generation records `speciesCompileArtifactId`, `phenotypeCompileArtifactId`, `compileArtifactSnapshot`, optional usage guide id/revision, `generationRecipe`, and `promptSnapshot` as captured provenance, not a runtime attempt. | Content immutable after creation; lifecycle metadata is limited to `status + feedback`, while `Phenotype.currentAcceptedVersion` points at the one accepted version when one exists. | Application service or CLI generated-output lifecycle flows. | `graphs/<graph_id>/phenotypes/` payload | `PhenotypeCompileArtifact`, `PhenotypeUsageGuide`, `GenerationJob`, `ReviewRecord`, `AssetIndex` |
| `GenerationJob` | output provenance model | Records one execution attempt, provider/tool or managed-runner summary, sanitized inputs, bounded request/output evidence, verification summary, failure category, and artifact IDs. Phenotype jobs bind to node/phenotype provenance; reference jobs bind to graph or species-group targets without synthetic phenotype records. | Runtime job/audit record. `executionMode` and `provenanceLevel` distinguish compiled-only, external-linked, runner-recorded, and runner-verified evidence. | Application service or CLI provider/generation flows. | `graphs/<graph_id>/generation-jobs/` | `PhenotypeVersion.generationRecipe`, `EntityCompileArtifact`, provider/runner contracts |
...
`GenerationJob` is an execution attempt and may fail. Phenotype jobs can produce `PhenotypeVersion` records, while reference jobs stay scoped to graph or species-group targets and may only link safe `AssetIndex` pointers. Managed execution stores bounded `requestEvidence`, `outputEvidence`, and `verificationSummary` on the job; it does not create a separate run object. External reference completion can move a reference job from `created` to `generated` only when active safe linked asset evidence exists, but that evidence remains `external-linked` and does not create phenotype lifecycle records. Reference asset migration updates current completion evidence to the new active pointer and keeps archived local pointers as historical evidence without changing the job prompt, target, input snapshot, or compile provenance. `PhenotypeVersion` is an immutable generated output snapshot; `generationRecipe` and `promptSnapshot` are provenance captured on that version.

### `PhenotypeUsageGuide` vs version feedback, reviews, and output metadata

`PhenotypeUsageGuide` is the stable usage contract for a `Phenotype`. It should not be stored as `PhenotypeVersion.feedback`, `ReviewRecord`, `OutputReference.metadata`, or copied into every `GenerationJob`. Compile artifacts, generation jobs, phenotype versions, and output references may reference the active guide id/revision and bounded summary they used. They do not become the guide source of truth, and generating a new version does not automatically rewrite the guide.
