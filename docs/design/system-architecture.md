...
Design Readiness 是 compile-time 设计语言完整度评估；readiness is embedded in CompileFrame，并随 `EntityCompileArtifact`、`SpeciesCompileArtifact` 或 `PhenotypeCompileArtifact` 的 dependency vector 一起解释 current/stale 状态。它不是 `Graph`、`SpeciesGroup`、`SpeciesNode`、`Phenotype` 或 context/facet 的 durable design fact，也没有独立 repository/cache table。默认 policy 是 `off | warn | block` 中的 `warn`：off 不评估、不提醒、不阻断；warn 只暴露 missing dimensions、warnings 和 suggestions；block 会在 missing critical readiness、stale readiness 或 blocking issue 存在时阻断 formal generation。`readiness assess --apply` 只能新增 compile/readiness artifact，不能回写上游 graph facts。

Self-Optimization 是反馈到维护候选的编排能力，不是自动进化图谱。它从 `PhenotypeVersion.feedback`、review-current notes、compile feedback/open questions 或人工摘要中提取可泛化候选，映射到 concept map 中的 usage guide、context、facet、species constraint、DesignRelationship contract 或 generation plan/task metadata，并默认输出 proposal/change-set preview。低置信度或作用范围不清的输入保留为 feedback/open question；apply 必须继续走现有 proposal/change-set/service 边界，不能由 extractor 直接修改正式 graph/context/facet/relationship facts。
