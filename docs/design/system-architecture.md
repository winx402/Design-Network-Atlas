...
当前仓库已完成 v0.4.1 local-first 实现，并通过 Phase 15 端到端验收：
...
v0.4.1 的完成边界是“本地优先试点可用并可被本地 API 集成”，不是生产级托管平台。npm CLI 发布、第一方真实模型 provider package、完整 Web 客户端、团队账户权限、审批流与多人同步服务属于 post-v1 路线。
