...
当前仓库已完成 v0.6.1 local-first 实现，并通过 Phase 16 端到端验收和 Phase 7 场景 skill 验收：
...
v0.6.1 的完成边界是“本地优先试点可用、可被本地 API 集成，能完成 preview 草案审阅确认闭环，并提供更有决策力的图谱建模/图谱编辑场景技能”，不是生产级托管平台。npm CLI 发布、proposal/batch 高阶建模、第一方真实模型 provider package、完整 Web 客户端、团队账户权限、审批流与多人同步服务属于 post-v1 路线。
