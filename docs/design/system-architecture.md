...
受管生成执行不引入 `GenerationRun`；`GenerationJob` 仍是一轮生成执行/尝试记录。Job 的 `executionMode` 表达 compiled-only、external-linked 或 managed-runner；`provenanceLevel` 表达 compiled-only、external-linked、runner-recorded 或 runner-verified；`requestEvidence`、`outputEvidence` 和 `verificationSummary` 只保存有界、脱敏、可复放的证据摘要。外部手工链接资产或 reference completion 不能提升到 runner-verified。MVP runner 包含 `mock-runner` 和 application/CLI adapter contract；`generation-job run` 记录 runner output evidence，`generation-job verify` 通过系统检查后才能提升为 runner-verified。`phenotype-version accept` 默认只警告 provenance 缺口，strict 模式可要求 linked `GenerationJob` 已通过 runner verification。
