...
Web 工作台通过 `/api/workbench/phenotypes` 读取本地数据，必须提供 loading、empty 和 error 状态。该 snapshot 包含 phenotypes、generationPlans 和 generationTasks 的只读视图。它不提供 accept / reject / archive / run task 等持久写按钮，也不直接写 graph、phenotype、review、impact、asset、output reference、plan/task 或 change-set。完整生产 Web 客户端、图谱编辑器、Web 审批、团队账户权限和多人同步界面属于 post-v1。
