...
- Phase 15 已补充本地 HTTP API baseline、`dna serve`、`dna sync export/import`、generation job/plan/task 导入导出和 API-backed 只读 workbench 数据加载。
...
- Web workbench 当前仍是只读本地工作台，不提供 durable write 按钮或 mutating endpoint 调用；完整生产 Web 客户端、团队账户/权限/审批/多人同步服务属于 post-v1。
