...
本目录是 DNA: Design Network Atlas 的项目设计与开发计划入口。当前本地优先版本已经按阶段完成到 Phase 28，并补强场景 skill、DNA Read-only Explorer、provider 失败脱敏和版本化交换格式，覆盖基础图谱、图谱建模/编辑/表型生成引导、图谱树输出、表型生成、审查影响分析、可选表型库、输出引用、结果库路由、preview change-set 审阅确认、Git 目录交换、本地 HTTP API 基线和生成 provider 基线；后续文档用于维护实现边界、测试策略和 post-v1 路线。
...
- 正式 compile 路径是 layered pipeline：`compile atlas|graph|group|species|phenotype` 产出 atlas -> graph -> group -> species-node -> phenotype frames、dependency vectors、feedback 和 open questions；preview 不落库，显式 apply/persist 才写入 generated trace artifacts。
- 正式 `phenotype generate` 路径以 layered `PhenotypeCompileArtifact` 为生成入口；preview 返回 species/phenotype compile artifacts、Phenotype、PhenotypeVersion、GenerationJob 和 prompt 且不落库，`--apply` 通过 application/CLI 边界一次性持久化运行记录。supplied stale artifacts 默认不能作为 current input，除非显式 historical replay。
- `PhenotypeUsageGuide` 已作为稳定绑定到 `Phenotype` 的使用说明对象：compile/generation 会记录 guide id、revision 和摘要；缺失 guide 只产生 warning，不阻断生成，也不会由新 `PhenotypeVersion`、`GenerationJob` 或 `OutputReference` 自动改写 guide。
- 生成计划层已补充 `PhenotypeGenerationPlan` 和 `PhenotypeGenerationTask`：plan 支持 graph/species-group/species-node/phenotype scope 并展开 planned phenotypes；task 可独立创建，`phenotype generate --task` apply 后会回写 compile artifact、GenerationJob 和 PhenotypeVersion links。
- 受管生成执行继续使用 `GenerationJob` 作为一次执行/尝试记录，不新增 run 表；job 记录 `executionMode`、`provenanceLevel`、有界 request/output evidence 和 `verificationSummary`。外部链接结果最多是 `external-linked`，只有 runner output 经过验证后才是 `runner-verified`，strict phenotype-version accept 可要求该证据。
...
- Phase 15 已补充本地 HTTP API baseline、`dna serve`、`dna sync export/import`、generation job/plan/task 导入导出和 API-backed 只读 Explorer 数据加载。
...
- Phase 28 已将 Web 收敛为 DNA Read-only Explorer：`/api/workbench/snapshot` 提供地图优先的 server-side view model，新增 `/api/workbench/graph-map`、`/api/workbench/graphs/:graphId`、`/api/workbench/generation` 和 `/api/workbench/library` 只读视图；Web 以 Atlas Map、Graph Explorer、Generation Board、Phenotype Library 和 Inspector 只读查看图谱关系、图谱内部结构、生成编排、结果库/图库、安全预览和 trace/detail 摘要。
- Web Explorer 当前仍是只读本地 Explorer，不提供 durable write 按钮或 mutating endpoint 调用；完整生产 Web 客户端、团队账户/权限/审批/多人同步服务属于 post-v1。
