...
- generation orchestration 测试覆盖 plan/task update preview/apply、immutable identity/trace links、batch selector safeguards、referenceGenerationJobIds/referenceAssetIds/contextReferenceIds 只保存 id、graph/group scoped reference generation、reference AssetIndex pointers，以及 export/import/security redaction。
