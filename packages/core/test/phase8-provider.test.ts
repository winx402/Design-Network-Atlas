...
describe("Phase 8 generation provider adapter boundary", () => {
  ...
  test("failed providers create a sanitized failed job and no asset pointers", async () => {
    const provider: GenerationProvider = {
      name: "failing-mock",
      async generate() {
        throw new Error(
          "provider unavailable for OPENAI_API_KEY=sk-do-not-store with password=hunter2 Authorization: Bearer runtime-secret https://signed.example.test/out.png?token=private"
        );
      }
    };
    ...
    expect(result.job.errorMessage).toBe("provider failure: provider-error");
    expect(result.job.outputSnapshot).toEqual({
    ...
    expect(JSON.stringify(result.job)).not.toContain("OPENAI_API_KEY");
    expect(JSON.stringify(result.job)).not.toContain("password");
    expect(JSON.stringify(result.job)).not.toContain("Bearer runtime-secret");
    expect(JSON.stringify(result.job)).not.toContain("signed.example.test/out.png");
    expect(result.assets).toEqual([]);
  });

  test("HTTP provider failures preserve safe status metadata without raw response text", async () => {
    const provider = new HttpGenerationProvider({
      name: "http-failing",
      endpoint: "https://provider.example.test/generate",
      headers: { Authorization: "Bearer runtime-secret" },
      fetcher: async () => new Response("OPENAI_API_KEY=sk-response-secret", { status: 503 })
    });

    const result = await runGenerationProvider({
      provider,
      generationJobId: "job-http-failed",
      graphId: "graph-provider",
      nodeId: "node-warning",
      phenotypeType: "image-prompt",
      taskBrief: "toolbar warning icon",
    ...
    expect(result.job.status).toBe("failed");
    expect(result.job.errorMessage).toBe("provider failure: http-error");
    expect(result.job.outputSnapshot).toEqual({
      provider: "http-failing",
      failure: {
        category: "http-error",
        httpStatus: 503,
        retryHint: "Provider returned HTTP 503. Retry after provider availability recovers."
      }
    });
    expect(JSON.stringify(result.job)).not.toContain("runtime-secret");
    expect(JSON.stringify(result.job)).not.toContain("sk-response-secret");
    expect(JSON.stringify(result.job)).not.toContain("OPENAI_API_KEY");
  });
